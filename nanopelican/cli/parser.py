import argparse

def default_parser():
    parser = argparse.ArgumentParser("Nano Pelican")

    parser.add_argument('--data_dir', type=str, required=True, help='Folder where the dataset is stored')
    parser.add_argument('--feature_key', type=str, required=True, help='What key the features of the database are stored under')
    parser.add_argument('--label_key', type=str, required=True, help='What key the labels of the database are stored under')

    parser.add_argument('--load', action=argparse.BooleanOptionalAction, help='Load entire datafiles into memory. Keeping it false only keeps single batch in memory at a time')
    parser.add_argument('--print_summary', action=argparse.BooleanOptionalAction, help='Prints model summary before execution')

    parser.add_argument('--experiment_root', type=str, default='experiments', help='Root folder where models and histories are saved')
    parser.add_argument('--experiment_name', type=str, default='experiment', help='Names of the experiments')

    parser.add_argument('--data_interpreter', type=str, default='None', help="If you need to index the features and labels further, implement an interpreter in nanopelican/data/interpreters")
    return parser

def test_parser():
    parser = default_parser()
    parser.add_argument('--models',required=True, type=str,  help='Name of the models to test (matches if models in name)')
    return parser.parse_args()

def train_parser():
    parser = default_parser()

    parser.add_argument('--n_hidden', type=int, help='Number of channels in the hidden layer')
    parser.add_argument('--n_outputs', type=int, help='Number of outputs (classification classes)')
    parser.add_argument('--activation', type=str, default='relu', help='Activation after every equivariant block (LinEq2v2 block)')
    parser.add_argument('--dropout_rate', type=float, default=0.0, help='Dropout rate (default 0)')
    parser.add_argument('--use_batchnorm', action=argparse.BooleanOptionalAction, help='Equips layers with batch normalization')

    parser.add_argument('--num_particles', type=int, default=-1, help='Number of particles to consider (and make the inner products of)')
    parser.add_argument('--num_particles_avg', type=int, default=1, help='Hyperparameter, should be approx equal to average number of particles')

    parser.add_argument('--data_format', type=str, help='How the data is formatted')

    parser.add_argument('--batch_size', type=int,  help='Batch size')
    parser.add_argument('--epochs', type=int,  help='Number of epochs')
    return parser.parse_args()

